...
np.save(
    Path('outputs', 'cache', 'automl-data_12m-desc_tr_cv_auroc.npy'),
...
np.save(
    Path('outputs', 'cache', 'automl-data_12m-desc_y.npy'),
    np.asarray(tpot_te_y)
)
np.save(
    Path('outputs', 'cache', 'automl-data_12m-desc_tpot_pred.npy'),
    np.asarray(tpot_te_pred)
)
np.save(
    Path('outputs', 'cache', 'automl-data_12m-desc_tpot_probas.npy'),
    np.asarray(tpot_te_probas)
)
np.save(
    Path('outputs', 'cache', 'automl-data_12m-desc_bag_pred.npy'),
    np.asarray(tpot_bag_te_pred)
)
np.save(
    Path('outputs', 'cache', 'automl-data_12m-desc_bag_probas.npy'),
    np.asarray(tpot_bag_te_probas)
)
np.save(
    Path('outputs', 'cache', 'automl-data_12m-desc_final_pred.npy'),
...
np.save(
    Path('outputs', 'cache', 'automl-data_12m-desc_choosen_pipeline.npy'),
    np.asarray(choosen_pipeline)
)

# tr_cv_auroc = np.load(Path('outputs', 'cache', 'automl-data_12m-desc_tr_cv_auroc.npy'))
# feature_importance = np.load(Path('outputs', 'cache', 'automl-data_12m-desc_featureimportance.npy'))
# tpot_te_y = np.load(Path('outputs', 'cache', 'automl-data_12m-desc_y.npy'))
# tpot_te_probas = np.load(Path('outputs', 'cache', 'automl-data_12m-desc_tpot_probas.npy'))
# tpot_te_pred = np.load(Path('outputs', 'cache', 'automl-data_12m-desc_tpot_pred.npy'))
# tpot_bag_te_probas = np.load(Path('outputs', 'cache', 'automl-data_12m-desc_bag_probas.npy'))
# tpot_bag_te_pred = np.load(Path('outputs', 'cache', 'automl-data_12m-desc_bag_pred.npy'))
# tpot_bag_calib_te_probas = np.load(Path('outputs', 'cache', 'automl-data_12m-desc_final_probas.npy'))
# tpot_bag_calib_te_pred = np.load(Path('outputs', 'cache', 'automl-data_12m-desc_final_pred.npy'))
# choosen_pipeline = np.load(Path('outputs', 'cache', 'automl-data_12m-desc_choosen_pipeline.npy'))
...
plt.title('12-month', loc='left', fontsize=18)
plt.xlabel('Predicted Risk of TRS', fontsize=14)
plt.ylabel('Proportion of Sample', fontsize=14)
plt.ylim([0, 0.45])
fig.savefig(Path('outputs', 'figs', 'proportion2thershold_12m.pdf'))
