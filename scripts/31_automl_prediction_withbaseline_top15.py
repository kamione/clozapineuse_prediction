...
top15_features = sorted_df['index'].tolist()[0:15]

# create numpy array for features and target
y = df.pop('is_cloz').to_numpy()
x = df[top15_features].to_numpy()
...
np.save(
    Path('outputs', 'cache', 'automl-data_baseline_top15-desc_tr_cv_auroc.npy'),
    np.asarray(tr_cv_auroc)
)
np.save(
    Path('outputs', 'cache', 'automl-data_baseline_top15-desc_featureimportance.npy'),
...
np.save(
    Path('outputs', 'cache', 'automl-data_baseline_top15-desc_tpot_pred.npy'),
    np.asarray(tpot_te_pred)
)
np.save(
    Path('outputs', 'cache', 'automl-data_baseline_top15-desc_tpot_probas.npy'),
    np.asarray(tpot_te_probas)
)
np.save(
    Path('outputs', 'cache', 'automl-data_baseline_top15-desc_bag_pred.npy'),
    np.asarray(tpot_bag_te_pred)
)
np.save(
    Path('outputs', 'cache', 'automl-data_baseline_top15-desc_bag_probas.npy'),
    np.asarray(tpot_bag_te_probas)
)
np.save(
    Path('outputs', 'cache', 'automl-data_baseline_top15-desc_final_pred.npy'),
...
np.save(
    Path('outputs', 'cache', 'automl-data_baseline_top15-desc_choosen_pipeline.npy'),
    np.asarray(choosen_pipeline)
)


# tr_cv_auroc = np.load(Path('outputs', 'cache', 'automl-data_baseline_top15-desc_tr_cv_auroc.npy'))
# feature_importance = np.load(Path('outputs', 'cache', 'automl-data_top15_baseline-desc_featureimportance.npy'))
# tpot_te_y = np.load(Path('outputs', 'cache', 'automl-data_baseline_top15-desc_y.npy'))
# tpot_te_probas = np.load(Path('outputs', 'cache', 'automl-data_baseline_top15-desc_tpot_probas.npy'))
# tpot_te_pred = np.load(Path('outputs', 'cache', 'automl-data_baseline_top15-desc_tpot_pred.npy'))
# tpot_bag_te_probas = np.load(Path('outputs', 'cache', 'automl-data_baseline_top15-desc_bag_probas.npy'))
# tpot_bag_te_pred = np.load(Path('outputs', 'cache', 'automl-data_baseline_top15-desc_bag_pred.npy'))
# tpot_bag_calib_te_probas = np.load(Path('outputs', 'cache', 'automl-data_baseline_top15-desc_final_probas.npy'))
# tpot_bag_calib_te_pred = np.load(Path('outputs', 'cache', 'automl-data_baseline_top15-desc_final_pred.npy'))
# choosen_pipeline = np.load(Path('outputs', 'cache', 'automl-data_baseline_top15-desc_choosen_pipeline.npy'))
